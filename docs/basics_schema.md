...
* If the type is one of the new java.time types such as local_date, local_date_time, zoned_date_time or instant, then the pattern should be described according to [java.time.format.DateTimeFormatter](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/format/DateTimeFormatter.html). If a time-zone is required such as when parsing an instant, you can add it within the pattern separated with a `@` character. Example: `pattern="yyyy-MM-dd HH:mm@Europe/Stockholm"`
...
This is called _implied decimal_ format.
