...
/**
 * Defines a schema for a fixed position buffer where the type of the line is
 * determined by the leading characters (control cell) at each line. Each cell
 * is defined by a fixed number of characters. Each line is separated by the
 * line separator defined in the base class {@link Schema}.
 * 
 ...
 * 
 * If the line separator is an empty string, the lines will be separated by the
 * sum of the length of the cells within the schema.
 * 
...
public class FixedWidthControlCellSchema extends FixedWidthSchema {
    ...
    /**
     * Creates a new schema which uses control cell with the supplied length and
     * allignment.
     * 
     * @param controlCellLength
     ...
     *         supplied line type.
     */
    public FixedWidthSchemaLine getSchemaLineByControlValue(
	...
	while (itLines.hasNext()) {
	    ...
	    FixedWidthSchemaLine schemaLine = getSchemaLineByType(line
		    .getLineType());
	    if (schemaLine == null)
	...
	FixedWidthControlCellSchema schema = (FixedWidthControlCellSchema) super
		.clone();
	return schema;
    }
